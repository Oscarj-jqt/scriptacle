<?php
    ...
    $stmt = $db->query("SELECT 
        theatre.id AS theater_id, 
        theatre.name AS theater_name, 
    ...
    JOIN 
        spectacles_parisiens ON representation.spectacle_id = spectacles_parisiens.id
    JOIN 
        avis ON spectacles_parisiens.id = avis.idSpectacle
    GROUP BY 
        theatre.id
    ORDER BY 
        avg_rating DESC;
        ...
        <div class="w-[400px] bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden p-6">
            ...
            <div class="p-4">
                ...
                <p class="text-center font-semibold text-gray-900 text-sm mt-2">
                    <?php echo htmlspecialchars($theater['presentation']); ?>
                </p>
                <div class="mt-4 flex justify-center">
                    <p class="text-sm font-semibold">Note Moyenne: <?php echo number_format($theater['avg_rating'], 2); ?></p>
                </div>
            </div>
        </div>
        <?php endforeach; ?>
